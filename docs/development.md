...
The install also configures worktree-local Lefthook hooks and the `dsh-translation-pairing` Git merge driver through `scripts/install-lefthook.mjs`. On OpenHarmony arm64, the same lifecycle maps the unavailable `tsgolint` platform package to its Linux arm64 package, validates and self-signs the native ELF files currently needed by the host workflow, including native Node addons, the mapped Lefthook executable, the mapped `tsgolint` executable, and compiled node-pty artifacts, restores their executable permission through `scripts/elf-self-sign.ts`, and injects the Homebrew library directory only into its child processes through `scripts/with-harmonybrew-libs.mjs`; pnpm skips Koffi's unsupported native install script, while `scripts/prepare-koffi.mjs` invokes the original Koffi build with Homebrew's `uname-is-linux` during the root `postinstall`, and `scripts/prepare-node-pty.mjs` invokes node-pty's original node-gyp build with the same scoped platform shim; other hosts skip these steps. The [OpenHarmony tsgolint mapping Agent Note](../.agents/notes/implemented/process/2026-08-16-openharmony-tsgolint-package-mapping.md) owns the package mapping; the [OpenHarmony node-pty source-build Agent Note](../.agents/notes/implemented/process/2026-08-16-openharmony-node-pty-source-build.md) owns the scoped source-build decision; the [worktree-local hooks Agent Note](../.agents/notes/implemented/process/2026-07-27-worktree-local-lefthook.md) owns the hook-path safety contract; the [automatic pairing merges Agent Note](../.agents/notes/implemented/process/2026-08-08-automatic-translation-pairing-merges.md) owns the merge driver.
