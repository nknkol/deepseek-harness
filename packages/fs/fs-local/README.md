...
- **Guarded creation uses a no-replace publication** — it prefers a hard link and uses Linux/OpenHarmony `renameat2(RENAME_NOREPLACE)` when the filesystem rejects hard links; an existing target remains protected and is reported through the normal guarded-create result.
