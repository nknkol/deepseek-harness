...
- **Lazy materialization.** `create(meta)` writes nothing; on the first `append`, the backend writes and `fsync`s the encoded header and first batch in a temporary file. POSIX publishes it with a no-replace primitive and `fsync`s the parent directory: it prefers a hard link, then uses Linux/OpenHarmony `renameat2(RENAME_NOREPLACE)` when the filesystem rejects hard links. Windows publishes it without overwrite via `MoveFileExW(..., MOVEFILE_WRITE_THROUGH)` and creates missing directories through the same write-through pattern. A created-but-never-appended session leaves nothing on disk and is absent from `list`.
...
- **One live writer per session** — append and repair are coordinated only inside the owning backend instance. Another backend instance or process must not write the same session until that owner reaches quiescent disposal. Initial same-id publication is collision-safe through the POSIX no-replace primitive or Windows write-through rename.
- **POSIX publication has a no-replace fallback** — first append prefers `link()` so same-id races fail instead of overwriting a committed log. POSIX-like filesystems that deny hard-link creation use Linux/OpenHarmony `renameat2(RENAME_NOREPLACE)`, which preserves the same no-overwrite guarantee on environments such as OpenHarmony HMFS.
